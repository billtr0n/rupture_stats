...
def yoffe_singularity_integral_weighted(Tr, dt):
    nu = Tr*np.sqrt(dt*(-dt+Tr)) + 2*np.sqrt(dt**3*(-dt+Tr))+(4*dt-Tr)*Tr*np.arcsin(np.sqrt(dt/Tr))
    de = dt**2*np.pi*Tr
    return nu/de
...
def yoffe(t, Tp, Tr, t0):
    ...
    val = np.mean([yoffe_singularity_integral_weighted(Tr, 0.1*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.2*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.3*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.4*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.5*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.6*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.7*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.8*dt),
                   yoffe_singularity_integral_weighted(Tr, 0.9*dt)])
...
def regularized_yoffe(t, Tp, Tr, t0):
    ...
    srf = srf[:int(np.ceil(Tr/dt))+1]
    hs = half_sin(t, Tp)*heaviside(Tp-t)
     
    # remove zero-padding from half-sine function
    n = int(np.ceil(Tp/dt))
    hs = hs[:n]
#    stf = fftconvolve(srf, hs, mode='full')
    tstf = convolve(srf, hs)
...
def compute_tarr( svm, dt ):
    ...
    cum_slip = cumtrapz(svm_norm, dx=dt)
